...
public class EliminarVenta extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ps = c.prepareStatement("update ventas set Habilitado=? where idProductos=?");
            ps.setString(1, "N");
            ps.setInt(2, Integer.parseInt(idp));
            int res = ps.executeUpdate();

            if (res > 0) {
                RequestDispatcher rd = request.getRequestDispatcher("Ventas?mensaje=Y");

                rd.forward(request, response);
            } else {
                RequestDispatcher rd = request.getRequestDispatcher("Ventas?mensaje=N");
