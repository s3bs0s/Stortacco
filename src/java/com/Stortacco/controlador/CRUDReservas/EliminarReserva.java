...
public class EliminarReserva extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ps = c.prepareStatement("update reservas set Habilitado=? where idReservas=?");
            ps.setString(1, "N");
            ps.setInt(2, Integer.parseInt(idp));
            int res = ps.executeUpdate();

            if (res > 0) {
                RequestDispatcher rd = request.getRequestDispatcher("Reservas?mensaje=Y");

                rd.forward(request, response);
            } else {
                RequestDispatcher rd = request.getRequestDispatcher("Reservas?mensaje=N");
